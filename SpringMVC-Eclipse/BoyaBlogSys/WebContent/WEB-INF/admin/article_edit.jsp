...
<body>
	<div class="page-container">
		<form class="form form-horizontal" id="form-article-add" method="post"  
			action="<%=path%>/article/edit_submit" >
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-2">
					<span class="c-red">*</span>文章标题：</label>
				<div class="formControls col-xs-8 col-sm-9">
				    <input type="hidden" class="input-text" value="${article.aid}" id="aid" name="aid">
					<input type="text" class="input-text" value="${article.title}" id="title" name="title">
				</div>
			</div>
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-2">
					...
					<span class="select-box">
						<select name="type" class="select">
							<c:forEach items="${Cateloglist}" var="s" varStatus="status">
								<c:if test="${s.gid==article.type}">
									<option value="${s.gid}" selected="selected">${s.gname}</option>
								</c:if>
								<c:if test="${s.gid!=article.type}">
									<option value="${s.gid}">${s.gname}</option>
								</c:if>
							</c:forEach>
						</select>
					</span>
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-9">
					<textarea name="content" cols="" rows="" class="textarea" dragonfly="true"
						placeholder="说点什么...最少输入10个字符" datatype="*10-8000"  nullmsg="备注不能为空！" 
						onKeyUp="$.Huitextarealength(this,8000)">${article.content}</textarea>
					<p class="textarea-numberbar"><em class="textarea-length">0</em>/200</p>
				</div>
			</div>
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-2">
					<span class="c-red">*</span>发布日期：</label>
				<div class="formControls col-xs-8 col-sm-9">
					<input type="text" class="input-text"  name="time" disabled="disabled"
						value="<fmt:formatDate value="${article.time}" pattern="yyyy-MM-dd HH:mm:ss"/>" />
					<input type="hidden" class="input-text" name="time" value="${article.time}" />
				</div>
			</div>
			<div class="row cl">
	...
	<button style="visibility: hidden" class="btn btn-primary btn-lg" 
		data-toggle="modal" data-target="#myModal2" id="dialog"></button>
	<!-- 模态框（Modal） -->
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<button type="button" onClick="layer_close();" class="btn btn-default" data-dismiss="modal">关闭</button>
				</div>
			</div>
		</div>
	</div>
