...
<body>
	...
	<div class="page-container">
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="r">共有数据：<strong>${comment_total}</strong> 条</span>
		</div>
		<div class="mt-20">
			<table class="table table-border table-bordered table-hover table-bg table-sort">
				...
				<tbody>
					<c:forEach items="${queryVo}" var="c" varStatus="status">
						<tr class="text-c">
							<td>
							...
							<td class="text-l"><div class="c-999 f-12">
								<div>文章名称:${c.aname}</div>
								<div>留言内容:${c.content}</div>
							</td>
							<td class="td-manage">
								<a title="删除" href="javascript:;" onclick="comment_del(this,${c.cid})" class="ml-5" 
		...
		function comment_del(obj,id){
			...
			if(supername == null || supername == ""){
				...
				layer.confirm('确认要删除吗？',function(index){
					...
					$.ajax({
						type:"post",
						url:"<%=path%>/comment/deleteByCid/"+cid,
